...
@Slf4j
public class LagersController extends AbstractViewController<Node> {

  private final EventBus eventBus;
  private final WebViewConfigurer webViewConfigurer;
  private final ClientProperties clientProperties;
  public Pane lagersRoot;
  public WebView lagersWebView;
  public Control loadingIndicator;
  private final ChangeListener<Boolean> loadingIndicatorListener;

  public LagersController(EventBus eventBus, WebViewConfigurer webViewConfigurer, ClientProperties clientProperties) {
  ...
  public void initialize() {
    lagersWebView.setContextMenuEnabled(false);
    webViewConfigurer.configureWebView(lagersWebView);
    ...
    lagersWebView.getEngine().getLoadWorker().stateProperty().addListener((obs, oldState, newState) -> {
  ...
  private void loadTournaments() {
    JavaFxUtil.runLater(() -> {
      ...
      lagersWebView.getEngine().load(website.getLagersHubUrl());
    });
  }
